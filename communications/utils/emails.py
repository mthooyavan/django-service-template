...
class Email(EmailMultiAlternatives):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        to = (
            ...
            if bool(to) and isinstance(to[0], str)
            else self._get_emails(to)
        )
        bcc = (
            self._validate_emails(bcc)
            if bool(bcc) and isinstance(bcc[0], str)
            else self._get_emails(bcc)
        )
        cc = (
            self._validate_emails(cc)
            if bool(cc) and isinstance(cc[0], str)
            else self._get_emails(cc)
        )
    ...
    def with_file(
        self, file_name: str, file_buffer: io.StringIO, mime_type: str
    ) -> "Email":
        """
    ...
    def _with_a_file(
        self, file_name: str, file_buffer: io.StringIO, file_type: File
    ) -> "Email":
        """
    ...
    def _override_recipients(
        self, to: List[str], bcc: List[str], cc: List[str]
    ) -> None:
        """
